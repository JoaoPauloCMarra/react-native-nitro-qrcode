...
std::string HybridQRCode::getMatrixPackedBase64(
    ...
    bool boostEcl) {
  std::lock_guard<std::mutex> lock(mutex_);
  return generator_.getMatrixPackedBase64(value, makeMatrixOptions(errorCorrectionLevel, minVersion, maxVersion, mask, boostEcl));
}

double HybridQRCode::getMatrixSize(
    const std::string& value,
    const std::string& errorCorrectionLevel,
    double minVersion,
    double maxVersion,
    ...
    bool boostEcl) {
  std::lock_guard<std::mutex> lock(mutex_);
  return generator_.getMatrixSize(value, makeMatrixOptions(errorCorrectionLevel, minVersion, maxVersion, mask, boostEcl));
}
...
::NitroQRCode::GenerateOptions HybridQRCode::makeOptions(
  ...
  options.logoAreaBorderRadius = toInt(logoAreaBorderRadius, "logoAreaBorderRadius");
...
::NitroQRCode::GenerateOptions HybridQRCode::makeMatrixOptions(
    const std::string& errorCorrectionLevel,
    double minVersion,
    double maxVersion,
    double mask,
    bool boostEcl) const {
  return makeOptions(512, 4, errorCorrectionLevel, "#000000", "#FFFFFF", minVersion, maxVersion, mask, boostEcl);
}
